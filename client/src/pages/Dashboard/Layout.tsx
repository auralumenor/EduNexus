...
const navItems = [
  ...
  { label: 'About LMS',    path: '/about',         Icon: Info },
];
      ...
      >
        <div className="flex items-center justify-between px-6 pb-8">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-primary rounded rotate-45 shrink-0 shadow-[0_0_15px_rgba(59,130,246,0.4)]" />
          ...
          {navItems.map(({ label, path, Icon }) => {
            ...
            return (
              <Link
                ...
                to={path}
                onClick={closeSidebar}
                className={`flex items-center gap-3 px-4 py-3 rounded font-medium text-sm transition-all duration-200 ${
                  isActive 
        ...
        <div className="px-3 pt-4 mt-2 border-t border-border-light dark:border-border-dark flex flex-col gap-1">
          ...
          <button  
            ...
            className="flex items-center gap-3 px-4 py-3 rounded font-medium text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 transition-all duration-200 w-full text-left"
          >
            <LogOut size={18} />
            <span>Logout</span>
          </button>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-8 bg-card-light dark:bg-card-dark border-b border-border-light dark:border-border-dark shrink-0">
          ...
          <Tooltip content="Navigate to your Profile & Security settings" delay={4000} position="bottom">
            <Link to="/profile" className="flex items-center gap-3 hover:bg-black/5 dark:hover:bg-white/5 py-1 px-2 rounded transition-colors cursor-pointer">
